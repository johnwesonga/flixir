defmodule FlixirWeb.SharedListLive do
  ...
  def mount(%{"tmdb_list_id" => tmdb_list_id}, _session, socket) do
    case Integer.parse(tmdb_list_id) do
      {list_id, ""} ->
        case Lists.get_shared_list(list_id) do
          {:ok, list} ->
            {:ok,
             socket
             |> assign(:list, list)
             ...
             |> assign(:error, nil)
             |> assign(:page_title, "#{list.name} - Shared List")}
          ...
          {:error, :not_shared} ->
            {:ok,
             ...
             |> assign(:movies, [])
             |> assign(:loading, false)
             |> assign(:error, "This list is not shared and cannot be viewed")
             |> assign(:page_title, "List Not Shared")}
  ...
  def handle_event("refresh_list", _params, socket) do
    case socket.assigns.list do
      ...
      list ->
        case Lists.get_shared_list(list.tmdb_list_id) do
          {:ok, updated_list} ->
            {:noreply,
             socket
              ...
              <div class="flex justify-between items-start">
                <div>
                  ...
                  <%= if @list.description && @list.description != "" do %>
                    <p class="text-green-100 text-lg"><%= @list.description %></p>
                  <% end %>
